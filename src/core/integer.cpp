...
void integer::set_ui(size_t i)				{ mpz_set_ui(val, i); }
...
integer& integer::operator= (size_t i) {
	if (is_initialized()) set_ui(i);
	else init_ui(i);
	return *this;
}
...
bool integer::operator== (size_t i) const 			{ 				return mpz_cmp_ui(val, i) == 0; }
...
bool integer::operator!= (size_t i) const			{ return not (*this == i); }
...
integer integer::operator+ (size_t i) const			{ integer a(*this); a += i;					return a; }
...
integer& integer::operator+= (size_t i) 			{ 				mpz_add_ui(val, val, i);	return *this; }
...
integer& integer::operator-= (size_t i)				{ 				mpz_sub_ui(val, val, i);	return *this; }
...
integer& integer::operator*= (size_t i)				{ 				mpz_mul_ui(val, val, i);	return *this; }
...
integer& integer::operator/= (size_t i)				{ 				mpz_div_ui(val, val, i);	return *this; }
...
integer integer::operator^ (size_t i)	 const		{ integer r(*this); r ^= i;					return r; }
...
long unsigned int integer::operator% (size_t i) const {
	mpz_t r;
	mpz_init(r);
	long unsigned int m = mpz_mod_ui(r, val, i);
	mpz_clear(r);
	return m;
}
...
long int integer::to_long_int() const {
	return mpz_get_si(val);
}

long unsigned int integer::to_uint() const {
